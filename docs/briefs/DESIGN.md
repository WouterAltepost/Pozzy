...
## 0. Revision 2026-09-23: Apple Human Interface Guidelines

Wouter asked for Pozzy to look and feel like an Apple product. This revision supersedes the "desk instrument" direction in sections 1 to 3 wherever they disagree; the component inventory in section 5 and the motion spec in section 9 still hold, with the values below. Source: the HIG pages for colour, typography, layout, materials, tab bars, toolbars, sidebars, lists, buttons, toggles, sheets, search and text fields, icons and dark mode (read 2026-09-23).

Direction. San Francisco through the system font stack. Content is inset grouped: white cards on the grouped grey (`#F2F2F7`), continuous corners, no borders, no shadows, hairline separators inset to the text. Chrome is the only translucent layer: the toolbar, the desktop sidebar, the phone tab bar and the toast sit on a blurred material and content scrolls under them. One tint, system blue, on everything interactive and nothing else: links, plain and gray buttons' text, the filled primary, selection, the switch, the focus ring, sidebar symbols. Green, orange and red are the system state colours. Area and mail account colours stay data. Dark mode is layered greys, black base on the phone (`#000` behind `#1C1C1E` cards), a lifted window grey on the desktop (`#1C1C1E` behind `#2C2C2E`).

Tokens (`web/src/style.css`). Text: `--ink` label, `--ink-2` 72%, `--ink-3` secondaryLabel 60%, `--ink-4` tertiaryLabel 30% (placeholders, chevrons). Fills: `--surface-2` tertiarySystemFill (controls, insets, hover), `--surface-3` systemFill (selection, pressed). Lines: `--line` separator 29%, `--line-2` opaqueSeparator. Tint `--brand` #007AFF / #0A84FF with `--brand-soft` at 12%. Material `--material` at 78% with `blur(40px) saturate(180%)`. Type: desktop is the macOS scale one step up for the browser (14 body, 13 subheadline, 12 footnote, 11 caption, 15 headline, 17 title 3, 22 title 2, 26 large title); under 720px the iOS text styles (17 body and headline, 15 subheadline, 13 footnote, 12 caption, 20 title 3, 22 title 2, 34 large title). Headings carry no letter-spacing; San Francisco tracks itself. Controls: 32px on the desktop, 44px on the phone; rows `--row-h` 36 / 44. Radii: 8 / 12 / 16 desktop, 10 / 14 / 20 phone, sheets 28px top corners, desktop dialogs 14px.

Shell. Desktop: a toolbar (52px) with the mark, the Capture command in the centre and account controls trailing; a sidebar source list at 220px with 30px rows, 13px text, symbols in the tint and a grey selection fill. Phone: no drawer; a floating tab bar on chrome material (Home, Agenda, Tasks, Mail, More) 16px in from the edges above the home indicator; More opens a sheet with the remaining destinations as an inset grouped list with tinted symbol tiles and chevrons, then appearance and sign out. The toolbar (44px) holds the Capture capsule and the appearance toggle. Page titles are large titles, 34 bold on the phone, 26 on the desktop.

Controls. Buttons: primary is filled tint with white text, secondary is the gray style (fill with tinted text), ghost is plain (tinted text), danger is tinted red, link is plain text; a bare `<button>` is gray. Checkboxes and radios are the Reminders circle: 22px ring in the separator grey, tint fill with a white mark when on. Text fields are the fill style with no border and a 3px tint ring on focus. Segmented control: iOS track and a white sliding segment with a soft shadow (grey in the dark). Badges and tags are capsules. Progress bars fill in the tint. Table heads are section headers: 11px uppercase secondaryLabel. Toast is a capsule on chrome material with a tinted action.

Icons stay Phosphor (weights match text; filled in the tab bar, regular elsewhere), the closest open set to SF Symbols. Geist stays as the fallback for non-Apple devices; no preload.
