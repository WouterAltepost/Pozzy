...
## 0. Revision 2026-09-23 (evening): modern, slick, premium

Wouter rejected the Apple HIG translation the same day ("do not like it, neither the desktop or the mobile version") and pointed at Erik Kennedy's "7 rules for creating gorgeous UI" and Taras Bakusevych's "58 rules for beautiful UI design". This revision supersedes the HIG direction and the earlier "desk instrument" direction wherever they disagree. Section 5's component inventory and section 9's motion spec still hold with the values below.

Direction, in Kennedy's terms. Black and white first: the neutrals are one cool hue (228) at varying brightness and the accent is that hue saturated, indigo `#4F6BFF` (`#7C93FF` in the dark), used only for actions, links, selection and the focus ring. Light comes from the sky: every plate has a 1px bright top edge, a hairline instead of a grey border and a soft shadow that falls downward; fields and tracks are inset wells with a shadow from the top lip; the primary button is brighter at the top with a coloured shadow beneath. Double the whitespace: page padding 40px, card padding 24px, gaps 20 to 24px, 40px rows on the desktop and 44px on the phone. Text pops and un-pops: page titles are 28px at weight 650 with tight tracking, eyebrows are 11px bold uppercase tracked grey (`.eyebrow`), card titles 15px semibold, meta 12px grey, big figures light and quiet (`.figure`). Geist stays as the typeface; Phosphor icons at the light weight (provided app-wide in `main.js`), filled only for the active tab.

Tokens (`web/src/style.css`). Light: `--bg` #F4F5F9, `--surface` #FFFFFF, wells `--surface-2` #EEF0F5 and `--surface-3` #E3E6EE, text `--ink` #0E1120, `--ink-2` #4B5163, `--ink-3` #7C8296, `--ink-4` #AEB3C2, hairlines `--line` at 7% and `--line-2` at 12% of ink. Dark: `--bg` #0B0C11, `--surface` #14161D, wells #1C1F28 and #262A36, text #F3F4F8 / #B4B8C7 / #7E8397 / #4F5467, hairlines white at 7% and 12%. `--edge` is the top highlight, `--inset` the well shadow, `--shadow-1..3` diffuse and downward, `--shadow-float` for floating chrome, `--chrome` the dark pill (#12141C light, #1C1F28 dark). Radii 8 / 10 / 16 / 22, pills for buttons, badges, tags, the segmented control and the tab bar. Type 14 body on the desktop, 15 on the phone; titles 28 / 30.

Review round (same evening). The primary button is quiet: a flat ink pill, no gradient, no coloured shadow; the accent stays on links, selection, checkboxes and progress. Phone navigation is the spring drawer again (menu button leading, the drawer from the left edge, drag to close), not a tab bar; TabBar.vue and MoreSheet.vue are gone. Navigation round (2026-09-23, later): the drawer is gone again, replaced by a floating glass tab bar (iOS 26 style, icons only) with Home, Tasks, Tracking and More; More is a bottom sheet with the remaining routes, Settings, the appearance switch and log out. Touch: on coarse pointers text links are 36px tall hit areas, icon buttons 40px, checkboxes 22px; phone tracker rows stack the name above seven 34px cells; the agenda status row keeps its count on one line and hides the "iCloud" label under 400px; the task form's three-column grid is two columns on the phone; the event form no longer repeats the dialog's title.

Shell. Desktop has no top bar: the sidebar (252px, hairline right edge, same background as the page) carries the mark and wordmark, the Capture command as an inset pill with the shortcut, three groups with eyebrows (Today: Home, Agenda, Tasks, Mail; Plan: Goals, Review, Capture; Track: Tracking, Hours, Study), Settings at the bottom, then the account row with a gradient avatar, the email, the appearance toggle and log out. The active item is a raised white plate with the accent on its icon. Phone: a slim material toolbar with the mark and the Capture pill; a floating glass tab bar (`TabBar.vue`) 14px above the home indicator: a 62px pill spanning the width with 16px margins (440px cap), `--surface` at 62% over a 28px blur, a `--material-edge` rim and `--edge` top light, four equal 50px icon-only slots (Home, Tasks, Tracking, More), no labels; one `--brand-soft` disc slides to the current slot on `--ease-spring` 360ms and the current icon turns to the fill weight in `--brand`; More stays lit on any route not on the bar; the bar drops below the edge while a text field has focus. More (`MoreSheet.vue`) is a `UiSheet` with inset grouped rows (48px, tinted symbol, chevron): the remaining routes, then Settings and a Dark mode switch, then Log out with the email.

Components. Buttons are pills: primary is an accent gradient lit from above with a coloured shadow, secondary is a raised plate, ghost is quiet text, danger tinted red. Fields are inset wells with eyebrow labels. Checkboxes are 18px rounded wells that fill with the accent gradient. Segmented control: inset pill track, raised sliding thumb. Dialogs and sheets use the double bezel: a tray in the well tone with a hairline holding the plate, concentric corners (22 plus 6). Toast is a dark pill. Progress bars are inset tracks with a lit accent fill. Table heads are eyebrows. Home widgets rise in one after the other on load (40ms stagger).
...
Mobile (390px, from the Claude Design handoff `pozzy-mobile-app-design/project/Pozzy Mobile.dc.html` and `Pozzy Phone.dc.html`, implemented 2026-09-16): the shell is the drawer variant of the design (Turn 2 used it for every screen). Top bar: the mark at the left (links home), a full-width capture pill ("Capture anything", opens the capture dialog). Navigation is the floating glass tab bar described under Shell (replaced the drawer on 2026-09-23); the theme toggle and log out live in the More sheet. The top bar, the tab bar and the sheet respect the iOS safe areas; `--tab-h` (76px under 1024px) keeps page content and toasts above the bar. Dialogs stay centred popups on the phone (the design's editor is a popup, not a sheet); only the briefing reply keeps the sheet.
...
| Phone tab bar (TabBar) | the active disc translates to the tapped slot over 360ms on `--ease-spring`; the icon scales to 0.88 on press; the bar slides down and fades over `--dur-ui` while the keyboard is up |
