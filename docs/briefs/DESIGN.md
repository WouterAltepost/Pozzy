...
- **UiLoadGate** page loading figure: `ready` prop, 140ms grace before the figure, content stays mounted but hidden and inert, reveal 280ms fade plus 320ms rise on the spring curve.
...
- **Mail**: PageHeader with counts, Sync now, accounts link; filters as a chip row; list rows 56px with account colour bar 3px, priority badge, from, subject, summary; detail in a large dialog.
...
| Load gate reveal | content opacity 280ms ease-out plus translateY(6px) to 0 over 320ms on the spring curve; figure mark breathes 1.4s, static under reduced motion |
