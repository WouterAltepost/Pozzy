...
- Minutes fields (task estimate, hours log) stepped 1, 6, 11, 16 because `min="1"` was the step base of a 5 minute step. Now `min="5" step="5"`.
- Loaders: the pressed control spins and the card shows a skeleton while Claude writes. Briefing Regenerate (skeleton over the text), Goals Suggest (skeleton list) and rollover link, Review Regenerate (skeleton over the reflection) and Refresh, Mail Reclassify, Mark handled and Create task (each its own spinner via a `pending` name), Capture Propose and Confirm per item (`busyId`), `ProposalEditor` and `TrackerForm` take a `busy` prop, Agenda accept buttons on the grid and the phone list disable and pulse while their item is placed (`deciding` prop), Accept all spins, phone Plan icon spins, Hours Add and Stop and log spin.
- Speed: the `api` service runs in Railway US East while the database is Supabase eu-west-1, so every query crosses the Atlantic and the home page's ten requests each pay it twice. Moving the service to an EU region is a Railway dashboard setting (Settings > Region), not code. In code: gunicorn `--threads 8` (was 4; ten widget requests plus a Claude call queued), tracker taps update the cell at once and the entry endpoints return the tracker's grid row (`week_row`) so nothing reloads the week, hours writes patch the list and refetch only the totals, the home tracker widget patches rows too.
- Trackers can be taken back and edited: `POST /api/trackers/<id>/untick` (bool toggles back, count and value subtract one step, the entry is removed at zero). Every tap shows a toast with Undo (`useToast().undo`, `UiToast` renders the button) that restores the previous value (`store.revert`); a long press or right-click on any cell or Today chip asks for the exact value, empty clears (`composables/useLongPress.js`); count and value cells show a small minus on hover on desktop. Same on the home "Trackers today" chips.
- Hours from the agenda: `GET /api/hours/suggestions` lists timed events of the last 7 days that have ended (not all-day, not Pozzy task blocks, 15 minutes to 12 hours) plus tasks completed without an estimate (complete_task already logs the ones with one), minus what is logged or dismissed. `POST /api/hours/suggestions/accept` creates the logs (idempotent per ref), `POST .../dismiss` remembers a skip. `hours_logs.source_ref` ("event:<uid>|<recurrence_id>" or "task:<id>") and table `hours_dismissals` (migration `h1000000hours`). Settings key `calendar_areas` (calendar url to area id) is edited under Settings > iCloud calendar, one area select per synced calendar, and prefills the area. UI: "From your agenda" card on the Hours page (per row editable minutes and area, Log and Skip, Log all and Skip all), a line on the home Hours widget with the pending count. Deterministic, no Claude. Test `api/tests/a/test_hours_suggestions.py`.
- Notes archived: hidden from the rail and the drawer (`hidden: true` in `router/index.js` NAV), the route and API stay so capture proposals of type note and old links still work.
...
- Second round: with the context present Claude still reasoned past the rules ("at 09:00 before your class at 09:30"). The standing rules now open every system prompt as hard constraints above the feature text, the planner payload repeats them as `rules`, and the model must write a `rule_check` line per item (gap against each rule) before choosing; only options that pass may be picked, otherwise -1. Third round: the model wrote the failed check but still returned option 0, so each pick now carries an explicit `place` boolean and `place: false` drops the item regardless of the index. Fourth round: the model's gap arithmetic was still wrong (19 minutes called "sufficient" against a 45 minute rule), so Claude now only states the minutes each rule requires per option (`checks`) and Python compares them with the real gaps, falls back to a passing option or drops the item. Fifth round (2026-09-17): an audit of the week-view plan still showed a task at 10:30 right after a workout and one wedged between two campus classes; the model had returned empty checks for those options and the three deterministic options were all next to appointments, so nothing compliant was left to fall back to. The rules now enter the slot search itself: a first call (`plan_constraints`) turns them into minutes per appointment, an earliest and latest hour and blocked days; Python pads the calendar with those numbers before computing free slots, so every option Claude ranks already complies. The `checks` mechanism is gone. Tests: buffers around a campus class and a workout, blocked day plus earliest hour.
...
- Second pass for the seven routes the handoff skipped (Goals, Mail, Capture, Study, Notes, Review, Settings): same title, meta and single-action pattern, add forms in dialogs (Study, Notes), collapsible filters and a paged list on Mail (40 rows plus Show more, the page went from 18110px to 3079px tall), stacked stage lists for applications, compact week navs, two-up Settings fields and a shorter job runs table. All seven verified at 390px: no overflow, zero console errors, dialogs open.
