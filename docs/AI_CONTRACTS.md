...
Settings `ai_rules` (list of sentences) and `ai_context` (free text), edited in Settings under "Rules for Pozzy". `_call` appends them to the system prompt of every feature as "## Standing rules from the user" and "## About the user", after the feature's own instructions, with the line that rules always apply and that an impossible item is left out with a reason. Empty when he wrote none. The planner honours a skip: `plan_week` may answer option -1 for an item and the caller drops it. Deterministic paths do not read the rules; hard limits for those live in the structured settings (working window, targets).
