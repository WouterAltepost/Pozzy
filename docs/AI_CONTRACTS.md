...
### `plan_constraints(payload: dict) -> dict | None`

Caller: `modules/ai/planner.py::_constraints`, before the free slots are computed, only when standing rules exist and the range has appointments. Kill switch: `ai_enabled.scheduling`. Model: smart. Prompt `plan_constraints`.

Input: `{"rules", "working_window", "range", "appointments": [{index, title, location, weekday, date, start, end}]}`. Output: `{"earliest", "latest", "blocked_days", "buffers": [{index, before, after, why}]}`. The planner pads each appointment by its buffer (never less than Settings `plan_buffer_minutes`), raises the window start to `earliest`, lowers the end to `latest`, and blocks whole days, all before `find_free_slots` runs. So every option `plan_week` ranks already respects the numeric side of the rules; the model classifies, the code does the arithmetic. Indexes outside the appointment list and malformed times are dropped. The response of `POST /api/ai/plan` carries the applied numbers as `constraints` for display and debugging.
