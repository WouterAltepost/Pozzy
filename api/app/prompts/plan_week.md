...
Output schema: `{"items": [{"id": "...", "place": true, "option": 0, "rule_check": "...", "reason": "..."}]}`. `place` false means no option passed the rules; the caller then drops the item whatever `option` says. `rules` in the payload repeats the standing rules so the check can quote them.
